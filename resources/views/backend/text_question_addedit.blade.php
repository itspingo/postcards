    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <form action="{{route('backend.text_question.'.$submit_path)}}" method="POST" ENCTYPE="multipart/form-data">
                        @csrf
                        <div class="card-header">
                            <div class="row" >
                                ...
                                <div class="col-3 text-end">
                                    <a href="{{route('backend.text_question')}}" class="btn btn-info btn-md"  >List Page</a>
                                </div> 
                            </div>
                        </div>
                        <div class="card-body">
...
<div class="col-md-6 mt-3">
	<div class="form-group">
		<label class="form-label" for="text_question_title">TEXT QUESTION TITLE</label>
		<input
			type="text"
			id="text_question_title"
			name="text_question_title"
			class="form-control"
			value="@if(isset($row->text_question_title)){{$row->text_question_title}}@endif"
			
		/>
	</div>
	<div id="diverr_text_question_title" text-danger></div>
</div>

<div class="col-md-6 mt-3">
	<div class="form-group">
    <label class="form-label" for="text_question_type">QUESTION TYPE</label>

@php
    
    if(!Empty($row)){ 
        $selected = $row->text_question_type; 
    }else{
        $selected = '';
    }
    $options = get_list_values('','');
    //pre($options);
	//$options = $text_question_type;
 @endphp

		 <select class="form-control" name="text_question_type" id="text_question_type"> 
		   <option value="">--Select--</option>
		   @foreach ($options as $key => $value)
			<option value="{{$key}}" {{($key == $selected) ? 'selected' : '' }}> 
				{{$value}} 
 ...
 <div id="diverr_text_question_type" text-danger></div>
</div>
<div class="col-md-6 mt-3">
	<div class="form-group">
    <label class="form-label" for="text_question_mandatory">MANDATORY</label>

@php
    
    if(!Empty($row)){ 
        $selected = $row->text_question_mandatory; 
    }else{
        $selected = '';
    }
    $options = get_list_values('','');
    //pre($options);
	//$options = $text_question_mandatory;
 @endphp

		 <select class="form-control" name="text_question_mandatory" id="text_question_mandatory"> 
		   <option value="">--Select--</option>
		   @foreach ($options as $key => $value)
			<option value="{{$key}}" {{($key == $selected) ? 'selected' : '' }}> 
				{{$value}} 
 ...
 <div id="diverr_text_question_mandatory" text-danger></div>
</div>

		<input
			type="hidden"
			id="text_question_foryesno"
			name="text_question_foryesno"
			value="@if(isset($row->text_question_foryesno)) {{ $row->text_question_foryesno }} @endif"
		/>
