...
class Card_designer extends Controller
{
    ...
    public function updateEffects(Request $request)
    {
        ...
        if ($card) {
            ...
            return response()->json(['message' => 'Effect updated successfully'], 200);
        } else {
            return response()->json(['message' => 'Card not found'], 404);
        }
    }
    ...
    public function updateMusic(Request $request)
    {
        ...
        if ($card) {
            $card->music_title = $request->music_title;
            $card->music_file = $request->music_file;
            $card->music_file_id = $request->music_file_id;
            $card->save();

            return response()->json(['message' => 'Music updated successfully'], 200);
        } else {
            return response()->json(['message' => 'Card not found'], 404);
        }
    }
